 ...
 * euslisp.cxx
 *
 * euslisp language module for SWIG.
 * ----------------------------------------------------------------------------- */

char cvsroot_euslisp_cxx[] = "$Id: euslisp.cxx 13963 2012-12-14 08:00:22Z wsfulton $";

#include "swigmod.h"
#include "cparse.h"
#include <ctype.h>

//#define EUSLISP_DEBUG
//#define EUSLISP_WRAP_DEBUG

static const char *usage = (char *) "\
EUSLISP Options (available with -euslisp)\n\
...
void EUSLISP::main(int argc, char *argv[]) {
  int i;

  Preprocessor_define("SWIGEUSLISP 1", 0);
  SWIG_library_directory("euslisp");
  SWIG_config_file("euslisp.swg");
...
int EUSLISP::top(Node *n) {
  ...
  Printf(lisp_filename, "%s%s.l", SWIG_output_directory(), module);
  ...
  if (CPlusPlus || CWrap) {
    ...
    Printf(clos_filename, "%s%s-clos.l", SWIG_output_directory(), module);
  ...
  Printf(f_runtime, "\n");
  Printf(f_runtime, "#define SWIGEUSLISP\n");
  Printf(f_runtime, "\n");
...
int EUSLISP::classHandler(Node *n) {
#ifdef EUSLISP_DEBUG
...
void EUSLISP::emit_defmethod(Node *n) {
  ...
  for (Parm *p = pl; p; p = nextSibling(p), argnum++) {
    ...
    String *ffitype = Swig_typemap_lookup("lispclass", p, ":integer", 0);
...
void EUSLISP::emit_getter(Node *n) {
  Node *parent = getCurrentClass();
...
int EUSLISP::memberfunctionHandler(Node *n) {
  // Let SWIG generate a global forwarding function.
  Setattr(n, "euslisp:memberfunction", "1");
  return Language::memberfunctionHandler(n);
}

int EUSLISP::membervariableHandler(Node *n) {
  // Let SWIG generate a get/set function pair.
  Setattr(n, "euslisp:membervariable", "1");
  return Language::membervariableHandler(n);
}

int EUSLISP::functionWrapper(Node *n) {
  ...
  if (CPlusPlus) {
    emit_defun(n, wname);
    if (Getattr(n, "euslisp:memberfunction"))
      emit_defmethod(n);
    else if (Getattr(n, "euslisp:membervariable")) {
      if (Getattr(n, "memberget"))
        emit_getter(n);
      else if (Getattr(n, "memberset"))
        emit_setter(n);
    }
    else if (Getattr(n, "euslisp:constructorfunction")) {
...
void EUSLISP::emit_defun(Node *n, String *name) {
  ...
  Printf(f_cl, "\n(defforeign %s %s\n	\"%s\"", name, module, func_name);
  String *ffitype = Swig_typemap_lookup("cout", n, ":integer", 0); // :pointer
...
int EUSLISP::constantWrapper(Node *n) {
...
int EUSLISP::variableWrapper(Node *n) {
  //  String *storage=Getattr(n,"storage");
...
int EUSLISP::typedefHandler(Node *n) {
...
int EUSLISP::enumDeclaration(Node *n) {
  String *name = Getattr(n, "sym:name");
  bool slot_name_keywords;
  String *lisp_name = 0;
  if (name && Len(name) != 0) {
    lisp_name = lispify_name(n, name, "'enumname");
    //Printf(f_cl, "#|", lisp_name);
    if (GetFlag(n, "feature:bitfield")) {
      Printf(f_cl, "\n(defbitfield %s", lisp_name);
    } else {
      Printf(f_cl, "\n(defcenum %s", lisp_name);
    }
...
void EUSLISP::emit_class(Node *n) {

#ifdef EUSLISP_WRAP_DEBUG
  ...
  // Printf(slotdefs, ";; ");
  //        String *ns = listify_namespace(Getattr(n, "euslisp:package"));
  String *ns = NewString("");
#ifdef EUSLISP_WRAP_DEBUG
  ...
  //   String *ns_list = listify_namespace(Getattr(n,"euslisp:namespace"));
...
// Includes structs
void EUSLISP::emit_struct_union(Node *n, bool un = false) {
#ifdef EUSLISP_DEBUG
  ...
  if (un) {
    Printf(f_cl, "\n(defcunion %s", lisp_name);
  } else
    Printf(f_cl, "\n(defcstruct %s", lisp_name);
...
void EUSLISP::emit_export(Node *n, String *name) {
  if (GetInt(n, "feature:export"))
    Printf(f_cl, "\n(cl:export '%s)\n", name);
}

void EUSLISP::emit_inline(Node *n, String *name) {
  if (GetInt(n, "feature:inline"))
    Printf(f_cl, "\n(cl:declaim (cl:inline %s))\n", name);
}

String *EUSLISP::lispify_name(Node *n, String *ty, const char *flag, bool kw) {
...
String *EUSLISP::strip_parens(String *string) {
...
String *EUSLISP::trim(String *str) {
  char *c = Char(str);
...
String *EUSLISP::infix_to_prefix(String *val, char split_op, const String *op, String *type) {
...
*/
String *EUSLISP::convert_literal(String *literal, String *type, bool try_to_split) {
  ...
  if (SwigType_type(type) == T_FLOAT || SwigType_type(type) == T_DOUBLE || SwigType_type(type) == T_LONGDOUBLE) {
    ...
    if (sscanf(s, "%d >> %d", &i, &j) == 2) {
      String* result = NewStringf("(ash %d -%d)", i, j);
      Delete(num);
      return result;
...
String *EUSLISP::lispy_name(char *name) {
  bool helper = false;
...
extern "C" Language *swig_euslisp(void) {
  return new EUSLISP();
}
